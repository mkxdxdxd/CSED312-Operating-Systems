  ...
  if (thread_mlfqs)
    {
        ...
        mlfqs_increment(); //increase recent_cpu by 1
...
void control_ready_list(struct thread *t)
{
...
thread_set_priority (int new_priority) 
{
  ...
  struct thread *cur = thread_current();
    ...
    {
        if (new_priority < list_entry(list_max(&cur->donators, less_priority, NULL), struct thread, doelem)->priority)
        {
    ...
    if (!list_empty(&ready_list))
    {
        if (cur->priority < list_entry(list_max(&ready_list, less_priority, NULL), struct thread, elem)->priority)
            thread_yield();
    }
}
...
int compare_list_max(struct list to_compare)
{
    ...
    return list_entry(maximum, struct thread, doelem)->priority;
...
{ 
  mlfqs_priority(t, NULL); //update_priority
...
mlfqs_priority(struct thread*t, void *aux)
{
  if (t == idle_thread)
    ...
    int new_priority = fixed_to_int(fixed_minus_int(fixed_minus_fixed(int_to_fixed(PRI_MAX), fixed_div_int(t->recent_cpu, 4)), 2 * t->nice)); 
...
next_thread_to_run (void) 
{
  if (list_empty (&ready_list))
    ...
    {
        struct thread *maximum = list_entry((list_max(&ready_list, less_priority, NULL)), struct thread, elem);

        list_remove(list_max(&ready_list, less_priority, NULL));
        return maximum;
    }
}
...
{
  struct list_elem *eleme;
  struct thread *cur = thread_current ();

  for (eleme = list_begin (&cur->donators); eleme != list_end (&cur->donators); eleme = list_next (eleme)){
