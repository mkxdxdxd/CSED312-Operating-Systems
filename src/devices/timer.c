...
timer_sleep (int64_t ticks) 
{
  ...
  old_intr_level = intr_disable(); //interrupt disabled
  current = thread_current();

  current->wake_ticks = start + ticks;
  thread_sleep(); //to put the thread into the sleep list
  intr_set_level(old_intr_level); //interrupt
}
